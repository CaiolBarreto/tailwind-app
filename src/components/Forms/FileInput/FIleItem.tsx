  ...
  variants: {
    state: {
      ...
      error: {
        ...
        deleteButton: 'text-error-700 hover:text-900',
      },
    },
  },
      ...
      {state === 'error' ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          <div className="flex flex-col">
            <span className="text-error-700 text-sm font-medium">{name}</span>
            <span className="text-error-600 text-sm">{formatBytes(size)}</span>
          </div>

          <button className="hover:text-error-600 text-error-700 text-sm font-semibold">
            Try Again
          </button>
        </div>
      ) : (
